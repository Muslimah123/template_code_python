...
class BookRecords:
    ...
    def __str__(self) -> str:
        ...
        """
        output=[]
        output.append("|    # | Date                | Customer   | Medication            | Quantity | Purchase Price | Prescription |")
        output.append("|------|---------------------|------------|-----------------------|----------|----------------|--------------|")
        for idex,transaction in enumerate(self.transaction,start=1):
            output.append(f"| {idx:4} | {datetime.fromtimestamp(transaction.timestamp).strftime('%Y-%m-%d %H:%M:%S')} | {transaction.customerID:10} | {transaction.name:21} | {transaction.quantity:8} | {transaction.purchase_price:14.2f} | {transaction.prescriptionID:12} |")
        return "\n".join(output)
    ...
    def reportOnPrescriptions(self) -> str:
        ...
        precription_report={} #Dictionary to store prescription data

        for transaction in self.transactions:
            if transaction.prescriptionID not in prescription_report:
                precription_report[transaction.prescriptionID]={
                    "medications":[],
                    "total_price":0
                }
        #Add the medication to the prescription's list of medications
        prescription_report[transaction.prescriptionID]["medications"].append(transaction.name)
        #increment the total price of the prescription
        prescription_report[transaction.prescriptionID]["total_price"]+=transaction.purchase_price

        output=[]
        output.append("|    # | Prescription ID | Total Price |")
        output.append("|------|-----------------|-------------|")

        for idx,(prescription_id,data) in enumerate(prescription_report.items(),start=1):
            output.append(f"| {idx:4} | {prescription_id:15} | {data['total_price']:11.2f} |")
    ...
    def topNSales(self, start: datetime = datetime.strptime('1970-01-02', '%Y-%m-%d'), end: datetime = datetime.now(), n = 10) -> str:
        ...
        transactions = sorted(
            [transaction for transaction in self.transactions if start <= datetime.fromtimestamp(transaction.timestamp) <= end],
    ...
    def load(cls, infile: str) -> BookRecords:
        ...
        try:
            with open(infile,'r') as f:
                ...
                transactions=[Sale(**data) for data in transactions_data]
                return cls(transactions)
        except FileNotFoundError:
            return cls([])  #Return an empty object if the file doesn't exist
